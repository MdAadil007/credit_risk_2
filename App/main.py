...
with row2[2]:
    avg_dpd_per_delinquency = st.number_input('Avg DPD', min_value=0, max_value=10, value=2)

with row3[0]:
    # trained on a 0-1 fraction (delinquent_months / total_loan_months), not a 0-100 percentage
    delinquency_ratio = st.number_input('Delinquency Ratio', min_value=0.0, max_value=1.0, step=0.01, value=0.30, format="%.2f")
with row3[1]:
